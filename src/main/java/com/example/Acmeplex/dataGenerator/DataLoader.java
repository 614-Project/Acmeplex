...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        // Add Theater data
        // List<Theater> theaters = TheaterRequestGenerator.generateRequests().stream()
        //     .map(request -> new Theater(
        //         request.getName(), 
        //         request.getAddress()))
        //         .collect(Collectors.toList());
        //         theaters.forEach(theaterService::addTheater);

    //     // Add TheaterSeat data
    //     List<TheaterSeat> theaterSeats = TheaterSeatRequestGenerator.generateRequests().stream()
    //         .map(request -> new TheaterSeat(
    //             request.getAddress(),
    //             request.getNumberOfSeatPerRow(),
    //             request.getNumberOfRows()))
    //         .collect(Collectors.toList());
    //     theaterSeats.forEach(theaterSeatService::createTheaterSeat);

    //     // Add Show data
    //     List<Show> shows = ShowRequestGenerator.generateRequests().stream()
    //         .map(request -> new Show(
    //             request.getShowStartTime(),
    //             request.getShowDate(),
    //             request.getTheaterId(),
    //             request.getMovieId()))
    //         .collect(Collectors.toList());
    //     shows.forEach(showService::createShow);
    // }
}
}